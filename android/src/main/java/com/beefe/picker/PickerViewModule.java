...
public class PickerViewModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
  ...
  private static final String PICKER_SUB_TITLE_TEXT = "pickerSubTitleText";
  private static final String PICKER_SUB_TITLE_COLOR = "pickerSubTitleColor";
  private static final String PICKER_SUB_TITLE_TEXT_SIZE = "pickerSubTitleFontSize";
  ...
  public void _init(ReadableMap options) {
    ...
    if (activity != null && options.hasKey(PICKER_DATA)) {
      ...
      RelativeLayout rlBarLayout = (RelativeLayout) view.findViewById(R.id.rl_barLayout);
      TextView subTitleTV = (TextView) view.findViewById(R.id.txt_sub_title);
      RelativeLayout barTitleLayout = (RelativeLayout) view.findViewById(R.id.barTitleLayout);
      ...
      int btlViewHeight;
      // 副标题
      if (options.hasKey(PICKER_SUB_TITLE_TEXT)) {
        subTitleText = options.getString(PICKER_SUB_TITLE_TEXT);
      }
      if (TextUtils.isEmpty(subTitleText)) {
        btlViewHeight = barViewHeight;
      } else {
        btlViewHeight =
            barViewHeight + (int) (activity.getResources().getDisplayMetrics().density * 40);
      ...
      if (options.hasKey(PICKER_SUB_TITLE_TEXT_SIZE)) {
        int subTitleSize = options.getInt(PICKER_SUB_TITLE_TEXT_SIZE);
        subTitleTV.setTextSize(subTitleSize);
      }

      if (options.hasKey(PICKER_SUB_TITLE_COLOR)) {
        ReadableArray array = options.getArray(PICKER_SUB_TITLE_COLOR);
        int[] colors = getColor(array);
        subTitleTV.setTextColor(argb(colors[3], colors[0], colors[1], colors[2]));
      }

      RelativeLayout.LayoutParams btlParams = new RelativeLayout.LayoutParams(
          RelativeLayout.LayoutParams.MATCH_PARENT,
          btlViewHeight);
      barTitleLayout.setLayoutParams(btlParams);


      if (options.hasKey(PICKER_TOOL_BAR_BG)) {
        ReadableArray array = options.getArray(PICKER_TOOL_BAR_BG);
        int[] colors = getColor(array);
        barTitleLayout.setBackgroundColor(argb(colors[3], colors[0], colors[1], colors[2]));
      }
